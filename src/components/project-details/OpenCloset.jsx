    ...
    <div className="project-detail-container">
      ...
      <section className="detail-section">
        ...
        <ul className="detail-list">
          <li><strong>배경:</strong> 패스트 패션으로 인한 환경 오염(<M>연간 의류 폐기물 10만 톤</M>) 및 고가 의류 구매 부담 해결.</li>
          <li><strong>목표:</strong> 사용자 간 직접 대여(P2P)를 통해 자원 순환을 돕고 합리적인 패션 소비 문화를 조성.</li>
          <li><strong>시장 검증:</strong> 대학생 30명 대상 설문 결과, <M>87.5%</M>가 의류 대여를 통한 수익 창출 및 문제 해결 가능성에 긍정적 답변 확인</li>
        </ul>
      </section>
      ...
      <section className="detail-section">
        ...
        <ul className="detail-list bullet">
          <li>React 기반 클라이언트와 <strong>Spring Boot API 서버</strong>로 구성된 구조</li>
      ...
      <section className="detail-section">
        ...
        <div className="contribution-item">
          ...
          <ul className="detail-list bullet">
            <li>JWT 기반 <strong>Access/Refresh Token</strong> 발급 및 Spring Security를 활용한 단방향 암호화(BCrypt) 적용.</li>
          </ul>
          <div className="insight-block">
            서버 세션 제거 → <strong>수평 확장(Scale-out) 가능한 Stateless 아키텍처</strong> 확보
          </div>
        </div>
        ...
        <div className="contribution-item">
          ...
          <ul className="detail-list bullet" style={{ marginBottom: '16px' }}>
            <li>제목 / 설명 / 사이즈 / 지역 조건 검색 구현</li>
            <li><strong>JPQL Boolean Logic</strong> 기반 동적 쿼리 설계</li>
            <li><strong>개선:</strong> if-else 분기 제거, <G>단일 쿼리 구조로 통합</G></li>
            <li><strong>성과:</strong> <G>DB 호출 횟수 1회로 감소</G>, 코드 유지보수성 향상</li>
          </ul>
      ...
      <section className="detail-section">
        ...
        <div className="issue-card">
          ...
          <ul className="issue-details">
            <li><strong>Analysis:</strong> 사용자들은 실시간 위치 공유보다 실제 생활 반경 내 거래를 선호하며, 잦은 GPS 호출은 <M>배터리 소모</M>와 거부감을 유발함.</li>
            <li><strong>Solution:</strong> 가입 시 등록한 '행정동' 정보를 활용한 <M>정적 위치 필터링(Static Location Filtering)</M> 방식으로 전환.</li>
            <li><strong>Result:</strong> <G>개인정보 보호 수준 향상</G> + 앱 진입 속도 개선으로 사용자 경험 최적화.</li>
        ...
        <div className="issue-card" style={{ marginTop: '24px' }}>
          ...
          <ul className="issue-details" style={{ marginBottom: '16px' }}>
            <li><strong>Analysis:</strong> 제목, 내용, 사이즈 등 다중 조건 검색 시 if-else 분기문이 <M>기하급수적으로 증가</M>해 유지보수가 어려워짐.</li>
          </ul>
          <div className="insight-block" style={{ marginBottom: '16px' }}>
            <strong>Solution:</strong> JPQL의 Boolean Logic(<code>:param IS NULL OR...</code>)을 적용하여 <strong>단일 쿼리로 통합</strong> 설계.
          </div>
...
List<Board> searchBoards(
       ...
       @Param("size") String size
          ...
          <ul className="issue-details" style={{ marginTop: '16px' }}>
            <li><strong>Result:</strong> <G>DB 호출 횟수 1회로 최적화</G>, 코드 가독성 및 유지보수성 비약적 향상</li>
          </ul>
        </div>
      </section>
